...
class WeightedEdge(Edge):
    def __init__(self, src, dest, total_distance, outdoor_distance):
        #Initialize with parent class init method to set source and destination nodes
...
#        Source and destination of edge (not really necessary but maybe more clear)
        src = edge.get_source()
        dest = edge.get_destination()

#       If source or edge aren't in the graph raise ValueError
        if not (self.has_node(src) and self.has_node(dest)):
            raise ValueError('Node not in graph')
#       Otherwise, add the edge to the dictionary tracking source node: edges 
        else:
